...
const PostCard: FC<{ postData: Post; className?: string }> = ({
  postData,
  className,
}) => {
  return (
    <Card radius="lg" className={"border-none " + className}>
      ...
      <CardBody className="space-y-3">
        <div className="flex flex-row h-5 text-small text-default-600 items-center space-x-2 font-medium">
          <div>{postData.category}</div>
          <Divider orientation="vertical" />
          <div>{postData.datetime}</div>
        </div>
        <div className="text-primary-800 font-medium md:text-lg">
          <p>{postData.title}</p>
        </div>
        <div>
          <User
